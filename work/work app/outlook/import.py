...
class GUIApp:
    ...
    def import_function(self):
        try:
            ...
            workbook = openpyxl.load_workbook('path_to_combined_file/combined_file.xlsx')
            sheet = workbook['Sheet']

            # Open a CSV file to write
            with open('path_to_utils_folder/output_file.csv', 'w', newline='') as csvfile:
                csvwriter = csv.writer(csvfile)

                # Iterate over the specified columns, starting from column B
                for col in range(2, sheet.max_column + 1):
                    record = []
                    # Collect values from rows 2 to 5 in the current column
                    for row in range(2, 6):
                        cell_value = sheet.cell(row=row, column=col).value
                        record.append(cell_value)
                    # Write the collected record to the CSV file
                    csvwriter.writerow(record)
            
            messagebox.showinfo("Info", "CSV file created successfully.")
